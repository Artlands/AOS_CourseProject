
set style data histogram
set style histogram clustered
set boxwidth 1 relative
set xlabel "{/:Bold=42 Benchmarks}"offset 0,-12
set yrange [0:100]
set xrange [-0.5:12]
set ytics 10
set mytics 2
set xtics nomirror
set ytics nomirror
set ylabel "{/:Bold=42 Coalsescing Efficiency}" offset -9,-4
set style fill pattern 6 border 7
set style line 2 linewidth 4
set key autotitle columnhead
set key font "Helvetica Bold,28"
set key horizontal top center
#set key vertical top right
set grid noxtics lw 1.5 lc 15
set border lw 2
set xtics font "Helvetica Bold,30" rotate by 270 offset 3,-3
set ytics font "Helvetica Bold,30"
set bmargin 20
set lmargin 22
set tmargin 2
set key on
set xtic scale 0

plot "./arq.txt" using 2:xticlabels(1) lc 2 fs pattern 1 border 50,\
'' using 3:xticlabels(1) lc  7 fs pattern 7 border -1,\
'' using 4:xticlabels(1)  lc 53  fs pattern 6 border 7,\
'' using 5:xticlabels(1)  lc rgb "navy"  fs solid 1 noborder

#plot "Data/pnnl/ipc.txt" using 6:xticlabels(1) lc  7 fs pattern 7 border -1,\
#'' using 4:xticlabels(1) lc 53  fs pattern 1 border 7,\
# '' using 5:xticlabels(1)  lc rgb "navy"  fs solid 1 noborder
